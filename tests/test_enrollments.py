...
async def test_register_student_not_found(client: AsyncClient):
    response = await client.post(
        "/enrollments", json={"student_id": 999999, "course_id": 1}
    )
...
async def test_register_course_not_found(client: AsyncClient):
    response = await client.post(
        "/enrollments", json={"student_id": 1, "course_id": 999999}
    )
    assert response.status_code == 404
...
async def test_cancel_enrollment(client: AsyncClient):
    ...
    # Cancel
    resp2 = await client.delete(f"/enrollments/{enrollment_id}")
    assert resp2.status_code == 204


@pytest.mark.asyncio
async def test_cancel_not_found(client: AsyncClient):
    response = await client.delete("/enrollments/999999")
    assert response.status_code == 404
...
async def test_credit_limit_exceeded(client: AsyncClient, db: AsyncSession):
    ...
    # Use courses that don't have schedule conflicts
    # Set course 1 to 10 credits and course 2 to 10 credits (total would be 20 > 18)
    ...
    try:
        ...
        resp2 = await client.post(
            "/enrollments", json={"student_id": 1, "course_id": 2}
        )
        assert resp2.status_code == 409
